...
class ModelTrainer:
    ...
    def initiate_model_training(self) ->ModelTrainerArtifact:
        try:
            ...
            scores = self.get_scores(dataframe=train_dataframe_pred,
                             metric_names=self.model_trainer_config.metric_list)
            ...
            print(f"number of row in testing: {test_dataframe_pred.count()}")
            scores=self.get_scores(dataframe=test_dataframe_pred, metric_names=self.model_trainer_config.metric_list)
            test_metric_artifact = PartialModelTrainerMetricArtifact(f1_score[0][1],
                                                                    precision_score[1][1], 
                                                                    recall_score[2][1])
